...
export function buildWorkflowActionGuidance(workflowName: string, context: string): string {
  ...
  if (workflowName === "BugWorkflow") {
    ...
    lines.push(
      "- In IssueWorkflow, use `run_command` only for repository-local runtime, build, or test regressions; generic product issues should still prefer analysis and code/context gathering.",
    );
  }
